...
typedef struct t_node {
	...
	char mc_area;			// 結界ルートのエリアフラグ( 1:後方 -1:前方 0:外 )
		...
		switch( sc_phase ) {
			...
			for ( apz_node=spz_node; apz_node; apz_node=apz_node->mpz_next ) {
				...
				for ( apz_line=apz_node->mpz_line; apz_line; apz_line=apz_line->mpz_next ) {
					if ( apz_line->mpz_node->mc_done == 0 ) {
...
----------------------------------------------------------------------------**/
{
	...
	/* 始端の場合 */
	if ( spz_jack == spz_way ) {
		ac_gard = 3 - spz_jack->mpz_way1->mc_col; // 前方ノード対色を封印色として採用
	}
	else {
		ac_gard = 3 - spz_jack->mpz_way0->mc_col; // 後方ノード対色を封印色として採用

		/* 抜穴の前方ノードを後方ノードと同色化 */
		if ( spz_jack != spz_waz ) {
			if ( spz_jack->mpz_way1->mc_col == ac_gard ) fv_SwapExe(spz_jack->mpz_way1,3-ac_gard);
		}
	}
...
----------------------------------------------------------------------------**/
{
	...
	for ( apz_node=apz_hole; apz_node; apz_node=apz_node->mpz_way0 ) apz_node->mc_area =  1;
	...
	while( 1 ) {
		...
		/* 前方エリア到達で終了フラグ */
		if ( apz_new == apz_hole || apz_new->mc_area < 0 ) break;
	}
