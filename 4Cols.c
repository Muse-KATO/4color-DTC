...
typedef struct t_node {
	...
	char mc_area;			// 結界ルートの前方エリアフラグ(0,1)
	char mc_swap;			// スワップ試行結果
	char mc_done;			// 処理済フラグ(0,1)
	char mc_draw;			// 処理済フラグ(0,1)
} T_node;
...
static HPEN sx1_pen[7];			// ペン( [0]最終海岸線 [1]逐次海岸線 [2]接岸ライン [3]減色ノード [4]結界ルート [5]抜穴ノード [6]三つ巴 )
...
const WORD* cpt_ver	 = { L"四色問題 (v1.4)" };
...
----------------------------------------------------------------------------**/
{
	...
	sx1_pen[6] = CreatePen(PS_SOLID,3,RGB(255,  0,  0)); // 三つ巴
...
----------------------------------------------------------------------------**/
{
	...
	while( 1 ) {
		...
		if ( sc_step == 6 ) {
			switch( fc_Step6() ) {
			    case 0: sc_step = 7; continue;
			    case 1: sc_step = 7; break;
			}
		}
		else
		/* 終了処理 */
...
/**----------------------------------------------------------------------------
＠ 抜穴ノードの封印 */
char		// <R>状態( 0:一掃／減色 1:ＲＧガード迂回／封印 2:三つ巴 )
fc_Step4()
/*
	結界ルート上の抜穴ノード spz_jack を検出する。
	もし抜穴ノードが減色ノードの連鎖ルート経由点でない場合は、ＲＧガードが存在している可能性がある。
	よってその場合は、ＲＧガードで結界ルートを迂回する。
	ただし、結界ルートの前後エリアをＲＧガードが取り囲んでいない場合は、
	減色ノードをＹスワップすることでダイレクトに減色し、抜穴ノード一掃として返却する。
...
----------------------------------------------------------------------------**/
{
	...
	/* ＢＹ連鎖の経由ノードでない場合 */
	if ( apz_node->mc_swap != 4 ) {

		/* ＲＧガード迂回を試行 */
		if ( fc_GardChg(apz_node) ) return( 1 );
	...
	/* 抜穴の前後ノードを同色化 */
	     if ( spz_jack == spz_way ) { if ( spz_jack->mpz1_sea[0]->mc_col == ac_gard ) fv_SwapExe(spz_jack->mpz1_sea[0],3-ac_gard); }
	else if ( spz_jack == spz_waz ) { if ( spz_jack->mpz1_sea[1]->mc_col == ac_gard ) fv_SwapExe(spz_jack->mpz1_sea[1],3-ac_gard); }
	else				{ if ( spz_jack->mpz_way1->mc_col    == ac_gard ) fv_SwapExe(spz_jack->mpz_way1,   3-ac_gard); }
...
/*
	...
	もし取り囲まれていない場合は、ＲＧガード迂回は実施せずに返却する。
...
----------------------------------------------------------------------------**/
{
	...
	/* エリアフラグのセット */
	for ( apz_node=spz_node; apz_node; apz_node=apz_node->mpz_next ) apz_node->mc_area = 0;
	for ( apz_node=apz_hole; apz_node; apz_node=apz_node->mpz_way1 ) apz_node->mc_area = 1;
	...
	while( 1 ) {
		...
		/* 前方エリア到達で終了フラグ */
		if ( apz_new == apz_hole || apz_new->mc_area ) break;
	}
